...
APP_VERSION = "1.0.0"
PLATFORM_VERSION = "CardVector Platform v1.0"
...
BRAND = {
    ...
    "panel_hover": "#182B46",
    "sidebar": "#050A13",
    "sidebar_hover": "#101A2B",
    "border": "#26384F",
    "border_accent": "#6F5A2A",
    "blue": "#2563EB",
    "blue2": "#3B82F6",
    "blue_dark": "#1D4ED8",
    "gold": "#D6A935",
...
class PutnamOS(BaseTk):
    ...
    def build_styles(self):
        ...
        s.configure("Primary.TButton", font=self.ui_font("button", True), padding=12)
        s.configure("Secondary.TButton", font=self.ui_font("button"), padding=8)
        s.configure(
            "Treeview",
        ...
        s.configure("TProgressbar", troughcolor=BRAND["panel2"], background=BRAND["blue"], bordercolor=BRAND["border"])
    ...
    def build_ui(self):
        side = tk.Frame(self, bg=BRAND["sidebar"], width=245)
        side.pack(side="left", fill="y")
        side.pack_propagate(False)

        tk.Label(side, text="CARDVECTOR", bg=BRAND["sidebar"], fg=BRAND["text"],
                 font=self.ui_font("app_title", True)).pack(pady=(28, 0))
        ...
        for section, names in nav_sections:
            ...
            for name in names:
                b = tk.Button(
                    side, text=name, anchor="w", bg=BRAND["sidebar"], fg=BRAND["muted"],
                    activebackground=BRAND["sidebar_hover"], activeforeground=BRAND["text"], relief="flat",
                    bd=0, cursor="hand2", font=self.ui_font("button", True), padx=22, pady=9,
                    command=lambda n=name: self.show_page(n)
                )
                b.pack(fill="x", padx=12, pady=1)
                b.bind("<Enter>", lambda _e, btn=b: btn.configure(bg=BRAND["sidebar_hover"]) if btn not in self.nav_buttons.values() or btn.cget("bg") != BRAND["panel2"] else None)
                b.bind("<Leave>", lambda _e, btn=b: btn.configure(bg=BRAND["sidebar"]) if btn.cget("bg") != BRAND["panel2"] else None)
        ...
        self.statusbar = tk.Label(self, textvariable=self.status, bg=BRAND["sidebar"], fg=BRAND["muted"],
                                  anchor="w", font=self.ui_font("small"), padx=12, pady=6)
    ...
    def header(self, title, subtitle=""):
        ...
        if subtitle:
            tk.Label(self.main, text=subtitle, bg=BRAND["bg"], fg=BRAND["muted"], font=self.ui_font("body")).pack(anchor="w", padx=SPACING["page_x"], pady=(0, 18))
    ...
    def button_bar(self, parent, pad_x=18, pad_y=(0, 10)):
        ...
        canvas = tk.Canvas(shell, bg=BRAND["panel"], highlightthickness=0, height=42)
    ...
    def capture_page(self):
        self.header("Capture", "CardVector Capture Studio v2: acquire clean front/back card photos for CardUploader handoff.")
        wrap = tk.Frame(self.main, bg=BRAND["bg"])
        wrap.pack(fill="both", expand=True, padx=34, pady=0)
        ...
        self.primary_button(row1, "Capture Next Photo", self.capture_next_photo_ui).pack(side="left", padx=8)
        self.action_button(row1, "Capture Front", lambda: self.capture_side_ui("front")).pack(side="left")
        self.action_button(row1, "Capture Back", lambda: self.capture_side_ui("back")).pack(side="left", padx=8)
        ...
        self.label(
            note,
            "Captures JPEGs and manages sessions. CardUploader performs recognition. CardVector OS orchestrates pricing, inventory, and workflow.",
            9,
            BRAND["muted"],
            False,
            anchor="w",
        ...
        review = self.card(wrap, fill="x", pady=(0, 14), ipady=10)
        self.label(review, "CAPTURE REVIEW v2", 12, BRAND["gold"], True, anchor="w", padx=18, pady=(12, 4))
        summary = capture_session_summary()
        if summary:
            review_text = (
                f"Latest session: {summary['folder']}\n"
                f"Session updated: {summary['session_updated']}\n"
                f"Capture count: {summary['count']}\n"
                f"Front/back pairs complete: {summary['pair_count']}\n"
                f"Incomplete pairs: {summary['incomplete_pairs']}\n"
                f"Next expected side: {summary['next_side']}\n"
            )
        else:
            review_text = "No capture session found yet."
        tk.Label(
            review,
            text=review_text,
            bg=BRAND["panel"],
        ...
        filmstrip = tk.Frame(review, bg=BRAND["panel"])
        filmstrip.pack(fill="x", padx=18, pady=(0, 10))
        tk.Label(filmstrip, text="Recent capture filmstrip", bg=BRAND["panel"], fg=BRAND["gold"],
                 font=self.ui_font("label", True)).pack(anchor="w", pady=(0, 6))
        strip_canvas = tk.Canvas(filmstrip, bg=BRAND["panel"], height=92, highlightthickness=0)
        strip_scroll = ttk.Scrollbar(filmstrip, orient="horizontal", command=strip_canvas.xview)
        strip_inner = tk.Frame(strip_canvas, bg=BRAND["panel"])
        strip_id = strip_canvas.create_window((0, 0), window=strip_inner, anchor="nw")
        strip_canvas.configure(xscrollcommand=strip_scroll.set)
        strip_inner.bind("<Configure>", lambda _e: (strip_canvas.configure(scrollregion=strip_canvas.bbox("all")), strip_canvas.itemconfigure(strip_id, height=strip_inner.winfo_reqheight())))
        strip_canvas.pack(fill="x", expand=True)
        strip_scroll.pack(fill="x")
        if summary and summary.get("recent"):
            for idx, name in enumerate(summary["recent"], 1):
                tile = tk.Frame(strip_inner, bg=BRAND["panel2"], highlightbackground=BRAND["border"], highlightthickness=1, width=150, height=74)
                tile.pack(side="left", padx=(0, 8))
                tile.pack_propagate(False)
                side = "front" if idx % 2 else "back"
                tk.Label(tile, text=side.upper(), bg=BRAND["panel2"], fg=BRAND["gold"],
                         font=self.ui_font("small", True)).pack(anchor="w", padx=10, pady=(8, 2))
                tk.Label(tile, text=name, bg=BRAND["panel2"], fg=BRAND["muted"], font=self.ui_font("small"),
                         justify="left", wraplength=130).pack(anchor="w", padx=10)
        else:
            tk.Label(strip_inner, text="No recent JPEGs. Filmstrip is ready for thumbnail cards.", bg=BRAND["panel"],
                     fg=BRAND["muted"], font=self.ui_font("body")).pack(anchor="w")
        badge_row = tk.Frame(review, bg=BRAND["panel"])
        badge_row.pack(anchor="w", padx=18, pady=(0, 10))
        for badge in ["Captured", "Sent to CardUploader", "Pricing Complete", "Inventory Complete"]:
            self.label(badge_row, badge, 9, BRAND["gold"] if badge == "Captured" and summary else BRAND["muted"], True, side="left", padx=(0, 12))
        review_buttons = tk.Frame(review, bg=BRAND["panel"])
        review_buttons.pack(anchor="w", padx=18, pady=(0, 12))
        self.action_button(review_buttons, "Open Session Folder", self.open_latest_capture_session_folder).pack(side="left")
    ...
    def inventory_location_registry_panel(self, wrap):
        ...
        for column in columns:
            self.etb_location_tree.heading(column, text=headings[column])
        ...
        self.action_button(actions, "Generate Labels", self.inventory_generate_etb_labels).pack(side="left", padx=(14, 0))
        self.action_button(actions, "Open Label Folder", self.inventory_open_etb_label_folder).pack(side="left", padx=8)
    ...
    def inventory_refresh_etb_locations(self, select_code=""):
        ...
        for row in rows:
            ...
            item = self.etb_location_tree.insert("", "end", values=values)
    ...
    def inventory_generate_etb_labels(self):
        rows = etb_location_rows()
        if not rows:
            messagebox.showinfo("ETB Location Registry", "Create an ETB location before generating labels.")
            return
        try:
            result = generate_etb_label_files(rows)
            self.status.set(f"Generated {len(result['html_files'])} ETB label file(s).")
            messagebox.showinfo(
                "ETB Labels",
                "Printable ETB labels generated.\n\n"
                f"Folder:\n{result['label_dir']}\n\n"
                f"Index:\n{result['index_file']}",
            )
        except Exception as exc:
            messagebox.showerror("ETB Labels", str(exc))

    def inventory_open_etb_label_folder(self):
        ETB_LABEL_ROOT.mkdir(parents=True, exist_ok=True)
        try:
            os.startfile(ETB_LABEL_ROOT)
        except Exception as exc:
            messagebox.showinfo("ETB Labels", f"Label folder:\n{ETB_LABEL_ROOT}\n\nCould not open automatically:\n{exc}")
