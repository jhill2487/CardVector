...
def audit_new_listing(
    ...
    print("Putnam OS Pricing & Decisions")
    ...
    summary_path.write_text(
        f"Putnam OS v{APP_VERSION} - Pricing & Decisions\n"
        f"Rows: {len(rows)}\n"
...
def write_acquisition_job_metadata(job_path, rows=None, export_summary=None):
    ...
    summary_path.write_text(
        "\n".join([
            "Putnam OS Acquisition Intake Summary",
...
class PutnamOS(BaseTk):
    ...
    def import_latest_carduploader_export_ui(self):
        ...
        if not latest:
            messagebox.showinfo("Putnam OS Import", "No CardUploader CSV found in Imports, Downloads, or Incoming Files.")
            return
    ...
    def import_page(self):
        ...
        tk.Label(
            ...
            text=(
                ...
                "Putnam OS validates the file before handing it to Pricing & Decisions."
            ),
            bg=BRAND["panel"],
            fg=BRAND["muted"],
            font=("Segoe UI", 10),
    ...
    def import_carduploader_csv_path(self, path):
        try:
            ...
            messagebox.showerror("Putnam OS Import", f"Could not import CSV:\n{exc}")

    def proceed_import_to_listings(self):
        if not self.imported_carduploader_csv:
            messagebox.showinfo("Putnam OS Import", "Import a CardUploader CSV before continuing to Pricing.")
            return
        if self.imported_carduploader_summary:
        ...
        self.status.set("Imported CSV loaded in Pricing & Decisions. Ready to analyze.")
    ...
    def capture_page(self):
        ...
        self.label(review, "CAPTURE REVIEW", 12, BRAND["gold"], True, anchor="w", padx=18, pady=(12, 4))
        ...
        if summary:
            review_text = (
                ...
                "Recent images:\n"
                + ("\n".join(summary["recent"]) if summary["recent"] else "No JPEGs found.")
            )
        else:
        ...
        for badge in ["Captured", "Recognized", "Decision Ready", "Inventory Assigned"]:
    ...
    def settings_page(self):
        self.header("Settings", "Configure local Putnam OS settings.")
        wrap = tk.Frame(self.main, bg=BRAND["bg"])
        wrap.pack(fill="both", expand=True, padx=34, pady=0)
        ...
        self.action_button(app_btns, "About Putnam OS", self.show_about_dialog).pack(side="left")
    ...
    def save_obs_settings_ui(self):
        try:
            ...
            messagebox.showinfo("Putnam OS Settings", "OBS settings saved.")
        except Exception as exc:
            self.status.set("Could not save OBS settings.")
            messagebox.showerror("Putnam OS Settings", str(exc))
    ...
    def save_ebay_policies_ui(self):
        try:
            ...
            messagebox.showinfo("Putnam OS Settings", "eBay business policies saved.")
        except Exception as exc:
    ...
    def save_carduploader_settings_ui(self):
        try:
            ...
            messagebox.showinfo("Putnam OS Settings", "CardUploader URL saved.")
        except Exception as exc:
    ...
    def show_about_dialog(self):
        messagebox.showinfo(
            ...
            "Putnam OS owns Capture, Pricing review, Decision Engine, Inventory, "
            "Business Intelligence, and workflow guidance.\n\n"
            "CardUploader owns card recognition and listing generation.",
        )
...
""", encoding="utf-8")
    ...
    def confirm_listing_export_step(self, _phase, message):
        return messagebox.askyesno("Putnam OS Pricing & Decisions", message)
...
if __name__ == "__main__":
    print(f"{APP_NAME} v{APP_VERSION} - UI Polish + UI Lock")
