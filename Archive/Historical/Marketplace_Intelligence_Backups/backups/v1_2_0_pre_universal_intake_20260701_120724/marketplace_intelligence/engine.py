...
from .csv_import import ImportResult, import_active_listings_csv
...
class MarketplaceIntelligenceEngine:
    ...
    def import_csv(self, path: Path) -> ImportResult:
        return import_active_listings_csv(Path(path))
    ...
    def analyze_file(self, path: Path, output_dir: Path | None = None, analysis_only: bool = False) -> dict:
        imported = self.import_csv(path)
        results = self.analyze_import(imported)
        target_dir = output_dir or self.default_output_dir(Path(path))
        reports = write_reports(target_dir, Path(path), results, analysis_only=analysis_only)
        summary = summarize(Path(path), target_dir, results)
        return {
            "import": imported,
            "results": results,
            "summary": summary,
