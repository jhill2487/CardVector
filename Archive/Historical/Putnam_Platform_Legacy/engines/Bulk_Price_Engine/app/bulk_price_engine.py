...
ROOT = find_root()
PLATFORM_DIR = ROOT / "Putnam_Platform"
if str(PLATFORM_DIR) not in sys.path:
    sys.path.insert(0, str(PLATFORM_DIR))

from Decision_Engine.decision_engine import DecisionEngine  # noqa: E402
from Decision_Engine.pricing import money, money_str  # noqa: E402
...
def write_report(path, source_file, file_type, records, invalid, ladder, output_files):
    ...
    lines=[]
    lines.append(f"Putnam Bulk Price Engine v{VERSION}")
...
def run(source_path: Path) -> Path:
    ...
    decision_result = DecisionEngine(root=ROOT).evaluate_pricing(records, CONFIG_PATH)
    ladder = decision_result.ladder
    processed = decision_result.processed
    invalid = decision_result.invalid
...
def main():
    print(f"Putnam Bulk Price Engine v{VERSION}")
    print("Root:", ROOT)
    print("")
    import argparse
