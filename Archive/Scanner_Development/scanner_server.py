...
def serve_file(handler: BaseHTTPRequestHandler, path: Path) -> None:
    try:
        ...
        if path.name == "scanner_studio.html":
...
def main() -> None:
    ...
    host = str(cfg.get("server_host", "0.0.0.0") or "0.0.0.0")
    print(f"Putnam Scanner Studio label+scan server running at http://127.0.0.1:{port}")
    if host in ("0.0.0.0", ""):
        for ip in local_ipv4_addresses():
            print(f"iPhone / LAN URL: http://{ip}:{port}")
    else:
        print(f"Configured host URL: http://{host}:{port}")
