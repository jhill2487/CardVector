...
REPORT = ROOT / "runtime_v0_9_0_modern_name_suffix_report.csv"
...
TARGET_SETS = {
    ...
    "Mega Evolution": ["ME: Mega Evolution"],
...
for p in [TCG_DB, MANIFEST]:
    shutil.copy2(p, archive / f"{p.stem}_before_v0_9_0_{stamp}{p.suffix}")
...
for c in cards:
    ...
    for p in candidates:
        ...
        if c_name == p_name:
            confidence = 1.0
            reason = "v0.9.0 stripped product card-number suffix exact name"
        elif sim >= 0.88:
            confidence = round(0.88 + sim * 0.10, 3)
            reason = "v0.9.0 stripped product card-number suffix fuzzy name"
        else:
            skipped_name += 1
...
print("Installed v0.9.0 Modern Product Name Number Suffix Normalizer")
print(f"Extension version: {old_version} -> 0.9.0")
