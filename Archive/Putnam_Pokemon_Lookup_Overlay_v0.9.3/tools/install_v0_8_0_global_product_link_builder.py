...
REPORT = ROOT / "runtime_v0_8_0_global_link_report.csv"
SUMMARY = ROOT / "runtime_v0_8_0_global_link_summary.csv"
...
for p in [TCG_DB, MANIFEST]:
    shutil.copy2(p, archive / f"{p.stem}_before_v0_8_0_{stamp}{p.suffix}")

VARIANT_TERMS = [
    "energy symbol pattern", "friend ball", "love ball", "poke ball", "poké ball",
    "master ball", "ultra ball", "great ball", "team rocket", "cosmos holo",
    "cosmos", "reverse holofoil", "reverse holo", "holofoil", "stamped", "stamp",
    "prerelease", "pre release", "staff", "league", "promo"
]
...
def set_keys(v):
    ...
    keys.add(raw.replace("scarlet violet ", ""))
    keys.add(raw.replace("sword shield ", ""))
    keys.add(raw.replace("sun moon ", ""))
    keys.add(raw.replace("xy ", ""))
    keys.add(raw.replace("bw ", ""))
    keys.add(raw.replace("me ", ""))
...
cur.execute("drop table if exists putnam_tcgtracking_matches_v080")
cur.execute("""
create table putnam_tcgtracking_matches_v080 (
...
for r in old_rows:
    keys = r.keys()
    cur.execute("""
    insert or ignore into putnam_tcgtracking_matches_v080
    values (?, ?, ?, ?, ?, ?, ?)
    """, [
        r["putnam_card_id"],
...
inserted = no_candidate = name_mismatch = ambiguous = 0
...
for c in cards:
    ...
    if not candidates:
        ...
        report.append([c["putnam_card_id"], set_name, c["card_name"], c["printed_number"], "", "", "NO_CANDIDATE", ""])
        continue
    ...
    for p in candidates:
        ...
        if c_name == p_name:
            confidence = 1.0
            reason = "v0.8.0 exact set+number+canonical-name"
        elif sim >= 0.88:
            confidence = round(0.88 + (sim * 0.10), 3)
            reason = "v0.8.0 fuzzy set+number+high-name-similarity"
        else:
            name_mismatch += 1
            summary[set_name]["name_mismatch"] += 1
            report.append([c["putnam_card_id"], set_name, c["card_name"], c["printed_number"], p["product_id"], p["product_name"], "SKIP_NAME_MISMATCH", f"{sim:.2f}"])
            continue
        ...
        inserted += 1
        linked_this_card += 1
        report.append([c["putnam_card_id"], set_name, c["card_name"], c["printed_number"], p["product_id"], p["product_name"], "LINKED", f"{confidence:.2f}"])

    if linked_this_card:
        summary[set_name]["linked"] += 1

cur.execute("drop table if exists putnam_tcgtracking_matches")
cur.execute("alter table putnam_tcgtracking_matches_v080 rename to putnam_tcgtracking_matches")
tcg.commit()

with REPORT.open("w", newline="", encoding="utf-8") as f:
    w = csv.writer(f)
    w.writerow(["putnam_card_id","set_name","card_name","printed_number","product_id","product_name","status","score"])
    w.writerows(report)
...
print("Installed v0.8.0 Global Product/Image Coverage Builder")
print(f"Extension version: {old_version} -> 0.8.0")
