...
set "CAPTURE_APP=%PUTNAM_ROOT%\Putnam_Platform\capture\Putnam_Capture.py"
...
where py >nul 2>nul
if %ERRORLEVEL% EQU 0 (
  py "%CAPTURE_APP%" --host localhost --port 4455 --password "%OBS_PASSWORD%" %*
) else (
  python "%CAPTURE_APP%" --host localhost --port 4455 --password "%OBS_PASSWORD%" %*
)
