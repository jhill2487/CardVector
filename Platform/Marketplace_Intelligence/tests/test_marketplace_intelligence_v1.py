...
from marketplace_intelligence.csv_import import SOURCE_CARDUPLOADER, SOURCE_CUSTOM, SOURCE_EBAY, detect_source, import_listing_csv
...
def run_smoke_test() -> None:
    ...
    carduploader_sample = ROOT / "examples" / "carduploader_export_sample.csv"
    custom_sample = ROOT / "examples" / "custom_listing_sample.csv"
    with tempfile.TemporaryDirectory() as tmp:
        ...
        ebay_import = import_listing_csv(sample)
        assert ebay_import.source_type == SOURCE_EBAY
        assert ebay_import.ready
        assert len(ebay_import.listings) == 4

        result = engine.analyze_file(sample, output_dir=output, analysis_only=False)
        summary = result["summary"]
        reports = result["reports"]
        assert result["import"].source_type == SOURCE_EBAY
        assert summary.listings_imported == 4
        assert summary.listings_normalized == 4
        ...
        carduploader_import = import_listing_csv(carduploader_sample)
        assert carduploader_import.source_type == SOURCE_CARDUPLOADER
        assert carduploader_import.ready
        assert carduploader_import.listings[0].catalog_sku == "PKM-001"
        assert carduploader_import.listings[0].tcgplayer_product_id == "PROD-001"
        carduploader_output = tmp_path / "carduploader_reports"
        result = engine.analyze_file(carduploader_sample, output_dir=carduploader_output)
        assert result["import"].source_type == SOURCE_CARDUPLOADER
        assert result["summary"].listings_normalized == 3
        assert result["reports"]["bulk_revise_csv"] is None
        assert result["reports"]["carduploader_validation_report"].exists()
        assert result["reports"]["underpriced_candidates_report"].exists()
        assert result["reports"]["pricing_recommendations"].exists()
        assert not (carduploader_output / "ebay_bulk_revise_changed_only.csv").exists()
        ...
        custom_import = import_listing_csv(custom_sample, source_type=SOURCE_CUSTOM, custom_mapping=custom_mapping)
        assert custom_import.source_type == SOURCE_CUSTOM
        assert custom_import.ready
        assert custom_import.listings[0].title == "Pokemon TCG Test Card Alpha 001"
        custom_output = tmp_path / "custom_reports"
        result = engine.analyze_file(custom_sample, output_dir=custom_output, source_type=SOURCE_CUSTOM, custom_mapping=custom_mapping)
        assert result["import"].source_type == SOURCE_CUSTOM
        assert result["summary"].listings_normalized == 2
        assert result["reports"]["bulk_revise_csv"] is None
        assert result["reports"]["pricing_recommendations"].exists()
        assert not (custom_output / "ebay_bulk_revise_changed_only.csv").exists()
