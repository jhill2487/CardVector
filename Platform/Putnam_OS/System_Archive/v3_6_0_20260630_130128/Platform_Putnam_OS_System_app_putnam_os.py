...
class PutnamOS(BaseTk):
    ...
    def build_ui(self):
        ...
        for name in ["Capture", "Import", "Pricing", "Inventory", "Orders", "Home", "Sessions", "Content", "Shipping", "Analytics", "Settings"]:
            b = tk.Button(
    ...
    def import_page(self):
        self.header("Import", "Import completed CardUploader CSV files and prepare them for Listings.")
        wrap = tk.Frame(self.main, bg=BRAND["bg"])
        wrap.pack(fill="both", expand=True, padx=34, pady=0)

        import_card = self.card(wrap, fill="x", pady=(0, 16), ipady=12)
        self.label(import_card, "CARDUPLOADER CSV IMPORT", 12, BRAND["gold"], True, anchor="w", padx=18, pady=(12, 6))
        tk.Label(
            import_card,
            text=(
                "Select a completed CardUploader export. Putnam OS will read the CSV, "
                "check the listing fields needed by the Listings workflow, and remember "
        ...
        self.action_button(button_row, "Proceed to Listings", self.proceed_import_to_listings).pack(side="left", padx=10)

        summary_card = self.card(wrap, fill="x", pady=(0, 16), ipady=12)
        self.label(summary_card, "IMPORT SUMMARY", 12, BRAND["gold"], True, anchor="w", padx=18, pady=(12, 6))
        if self.imported_carduploader_summary:
            ...
            summary_text = "No CardUploader CSV imported yet." + last_text
        ...
        tk.Label(
            handoff,
            text=(
                "Proceed to Listings loads the imported CSV into the existing Pricing tab. "
                "Pricing rules, eBay export columns, shipping confirmation, and export logging "
                "remain handled by the existing Listings workflow."
            ),
            bg=BRAND["panel"],
            fg=BRAND["muted"],
            font=("Segoe UI", 10),
    ...
    def proceed_import_to_listings(self):
        if not self.imported_carduploader_csv:
            messagebox.showinfo("Putnam OS Import", "Import a CardUploader CSV before proceeding to Listings.")
            return
        if self.imported_carduploader_summary:
        ...
        self.status.set("Imported CSV loaded in Listings/Pricing. Ready to analyze.")
    ...
    def inventory_location_registry_panel(self, wrap):
        ...
        columns = ("location_code", "status", "assigned", "remaining", "capacity")
    ...
    def inventory_refresh_etb_locations(self, select_code=""):
        ...
        for row in rows:
            values = (
                row["location_code"],
                row["status"],
                row["estimated_assigned_count"],
                row["estimated_remaining_capacity"],
