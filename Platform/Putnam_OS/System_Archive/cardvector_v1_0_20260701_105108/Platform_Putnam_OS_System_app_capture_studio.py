...
OBS_AUTH_MISSING_MESSAGE = "OBS authentication is enabled, but no OBS password is configured in Putnam OS."
...
def _obs_status_from_error(exc: Exception, host: str, port: int) -> str:
    ...
    if "authentication" in lower and ("failed" in lower or "invalid" in lower or "denied" in lower):
        return "OBS status: auth failed. Check the OBS password configured in Putnam OS."
