...
def fake_obs_manager(service: CaptureStudioService):
    return capture_studio.OBSConnectionManager(
        settings_loader=capture_studio.load_capture_settings,
        password_loader=capture_studio.obs_password,
        obs_module_loader=lambda: service._obs_module(),
        error_factory=capture_studio.CaptureStudioError,
        auth_missing_message=capture_studio.OBS_AUTH_MISSING_MESSAGE,
        default_host=capture_studio.DEFAULT_OBS_HOST,
        default_port=capture_studio.DEFAULT_OBS_PORT,
    )
...
def test_obs_status_uses_configured_password(tmp_path: Path) -> None:
    ...
    service.obs_manager = fake_obs_manager(service)
    try:
...
def test_obs_status_and_capture_use_same_obs_config(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert connections == [("127.0.0.1", 4455, "same-config")]
