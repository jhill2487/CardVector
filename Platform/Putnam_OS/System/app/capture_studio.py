...
from obs_connection_manager import OBSConnectionManager
...
_SHARED_OBS_MANAGER: OBSConnectionManager | None = None
...
def get_shared_obs_manager() -> OBSConnectionManager:
    global _SHARED_OBS_MANAGER
    if _SHARED_OBS_MANAGER is None:
        _SHARED_OBS_MANAGER = OBSConnectionManager(
            settings_loader=load_capture_settings,
            password_loader=obs_password,
            obs_module_loader=_load_obs_module,
            error_factory=CaptureStudioError,
            auth_missing_message=OBS_AUTH_MISSING_MESSAGE,
            default_host=DEFAULT_OBS_HOST,
            default_port=DEFAULT_OBS_PORT,
        )
    return _SHARED_OBS_MANAGER
...
class CaptureStudioService:
    def __init__(
        ...
        obs_manager: OBSConnectionManager | None = None,
    ):
        ...
        self.obs_manager = obs_manager or get_shared_obs_manager()
    ...
    def obs_status(self) -> str:
        status = self.obs_manager.status(check=True, timeout=3)
    ...
    def capture_obs_jpeg(self) -> bytes:
        ...
        scene_name = self.obs_manager.current_program_scene(settings=settings, timeout=5) or str(settings.get("obs_scene", DEFAULT_OBS_SCENE))
        jpeg_quality = int(settings.get("jpeg_quality", DEFAULT_JPEG_QUALITY))
        response = self.obs_manager.source_screenshot(
            scene_name,
            "jpg",
            None,
            None,
    ...
    def _obs_client(self, settings: dict[str, Any] | None = None, timeout: int = 5) -> Any:
        return self.obs_manager.get_client(settings=settings, timeout=timeout)
