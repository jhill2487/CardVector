...
def audit_new_listing(
    ...
    if use_market:
        if progress_callback:
            progress_callback("Running comp search...", percent=95, current=len(rows), total=len(rows))
        ...
        if progress_callback:
            progress_callback(
                f"Search complete: {comp_total_accepted} accepted, {comp_total_rejected} rejected.",
                percent=98,
                current=len(rows),
                total=len(rows),
            )
    ...
    def auto_run(self):
        ...
        try:
            ...
            if hasattr(self, "flow_var"):
                self.flow_var.set("Running comp search...")
            self.status.set("Running comp search...")
