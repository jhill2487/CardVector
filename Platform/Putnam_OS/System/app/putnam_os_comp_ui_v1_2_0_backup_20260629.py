...
EXPORT_FLOOR_PRICE = Decimal("0.89")
...
def summarize_final_prices(final_prices, batch_location, output_csv_path):
    ...
    cart_sweeteners = sum(1 for price in final_prices if price < Decimal("1.00"))
    return {
...
def prepare_listing_export_rows(rows, batch_location, progress_callback=None):
    ...
    for idx, row in enumerate(rows, 1):
        ...
        cart_sweetener = final_price < Decimal("1.00")
        review = dict(r)
        review.update(
            {
                "optimizer_row": idx,
...
def latest_ebay_active_listings_report():
    folders = [ROOT / "eBay Store Items", ROOT / "Ebay Store Items"]
    candidates = []
...
def apply_inventory_audit_action(session, action, notes="", audit_root=None, capture_source=None):
    ...
    if action == "confirm":
        ...
        record["confirmed_location"] = session.get("batch_location", "")
...
def generate_inventory_audit_reports(session, audit_root=None):
    ...
    summary = [
        f"Putnam OS Inventory Audit Mode v1.0",
...
def audit_new_listing(
    ...
    if progress_callback:
        progress_callback("Loading CSV", percent=10)
    load_started = time.perf_counter()
    rows = read_csv(path)
    load_time = time.perf_counter() - load_started
    typ = detect_type(rows)
    if progress_callback:
        progress_callback("Validating rows", percent=20, current=0, total=len(rows))
    ...
    if progress_callback:
        progress_callback("Confirming shipping policy", percent=30, current=len(rows), total=len(rows))
    ...
    if progress_callback:
        progress_callback("Counting cart sweeteners", percent=65, current=len(rows), total=len(rows))
    ...
    if progress_callback:
        progress_callback("Confirming final export", percent=75, current=len(rows), total=len(rows))
    ...
    if progress_callback:
        progress_callback("Writing eBay CSV", percent=85, current=len(rows), total=len(rows))
    ...
    if progress_callback:
        progress_callback("Writing export log", percent=92, current=len(rows), total=len(rows))
    ...
    if use_market:
        if progress_callback:
            progress_callback("Checking market opportunities", percent=95, current=len(rows), total=len(rows))
...
class PutnamOS(BaseTk):
    ...
    def inventory_page(self):
        ...
        self.label(setup, "INVENTORY AUDIT MODE v1.0", 12, BRAND["gold"], True, anchor="w", padx=18, pady=(12, 4))
        self.label(
            setup,
        ...
        tk.Button(setup_actions, text="Load Audit Queue", bg=BRAND["blue"], fg="white",
                  activebackground=BRAND["blue2"], relief="flat", font=("Segoe UI", 11, "bold"),
        ...
        tk.Button(actions, text="Confirm", bg=BRAND["blue"], fg="white", activebackground=BRAND["blue2"],
        ...
        self.action_button(actions, "Missing Card", lambda: self.inventory_apply_action("missing")).pack(side="left", padx=8)
    ...
    def inventory_update_queue_view(self):
        ...
        self.inventory_stats_var.set(
            f"Audited: {stats['audited']}  Remaining: {stats['remaining']}  Confirmed: {stats['confirmed']}  "
            f"Already correct: {stats['already_correct']}  Missing: {stats['missing']}  "
            f"Needs review: {stats['needs_review']}  Skipped: {stats['skipped']}"
        )
    ...
    def launch_capture_studio(self):
        launcher = TOOLS / "Run_Putnam_Capture.bat"
        if not launcher.exists():
            messagebox.showwarning("Putnam OS", f"Capture Studio launcher not found:\n{launcher}")
            return
        try:
            subprocess.Popen([str(launcher), "--mode", "front", "--auto-capture"], cwd=str(ROOT))
            append_activity("Launched Capture Studio for inventory audit verification")
            self.status.set("Capture Studio launched. Confirmed audit cards will attach the latest capture image when enabled.")
        except Exception as exc:
            messagebox.showerror("Putnam OS", str(exc))
    ...
    def auto_run(self):
        ...
        try:
            if self.detected == "carduploader_new":
                ...
                if not batch_location:
                    raise ExportCancelled("Export canceled because batch/location was blank.")
                self.update_pricing_progress("Starting Listing Optimizer", percent=5, current=0, total=len(self.rows))
                ...
                try:
                    self.flow_var.set(f"âœ“ Loaded {rows} rows  â†’  âœ“ Optimized pricing  â†’  âœ“ Confirmed policies  â†’  âœ“ Output ready")
                    self.result_var.set(
                    ...
                    pass
                self.status.set(f"Complete. Optimized prices: {changes}. Cart sweeteners: {export_summary['cart_sweetener_count']}.")
                messagebox.showinfo(
                    "Putnam OS",
                    f"Analysis complete.\nRows: {rows}\nOptimized price changes: {changes}\n"
                    f"Cart sweeteners: {export_summary['cart_sweetener_count']}\n"
