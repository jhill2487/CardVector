...
| CV-OQ-004 | What is the permanent owner/purpose of `System/decision_engine`? | Avoids competing marketplace/alert decisions | Partially active checks, placeholders, stale paths, Marketplace UI access | Current outputs, user value, desired scope | Project owner + architect | Decision Engine migration | Keep isolated; no expansion |
